...
public:
	...
	Pearl(HGon parent, int pos) {
		this->parentHex = parent;
		this->position = pos;
		this->location[0] = sinf((-30.0f + 60.0f * (position - 1)) * PI / -180.0f) * 0.5f * hWidth;
		this->location[1] = cosf((-30.0f + 60.0f * (position - 1)) * PI / -180.0f) * 0.5f * hWidth;
	}
